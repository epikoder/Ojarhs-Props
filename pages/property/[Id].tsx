		...
		<React.Fragment>
			<div className="mb-4">
				...
				{status === 'success' && <div className="flex flex-col gap-5 lg:flex-row lg:justify-around justify-center ">
					<div>
						...
						<div className="md:hidden">
							<div>
								...
								<Card className="flex overflow-x-scroll p-1 mx-1 rounded-md">
									{data.video_galleries.concat(data.video).map((e, i) =>
										<video key={i} className="rounded-sm mx-1 h-24" src={resolveFilePath(e)} controls playsInline />)
									}
								</Card>
							</div>
							<Card className="text-left m-1 h-full">
								<div className="grid grid-cols-3 text-center items-center bg-red-600">
									<div className="text-xs text-white">
										<FullscreenIcon />
										<span className="uppercase mx-1">
					...
					<div className="lg:max-w-[50%] xl:max-w-[unset]">
				...
				{status === 'failed' && <Card className="flex flex-col justify-center items-center">
					Property Not Found
				</Card>}
			</div>
		</React.Fragment>
